...
-- Recheck Data
------- แก้ไข เงื่อนไขของการ Transpose
...
WITH ranked AS (
    SELECT 
        sbl_member_id,
        NTILE(100) OVER (ORDER BY CFG_Total DESC) AS rank_total
...
-- คำนวณปีและเดือน
SET @yyyy_lm = YEAR(DATEADD(MONTH, -1, GETDATE()));  -- ปีที่แล้ว
SET @yy_lm = YEAR(DATEADD(MONTH, -1, GETDATE())) % 100;  -- ปีที่แล้ว (สองหลัก)
SET @mm_lm = MONTH(DATEADD(MONTH, -1, GETDATE()));  -- เดือนปัจจุบัน
...
BEGIN
    ...
    CREATE TABLE MKTCRM.SEG.CRM_SINGLE_VIEW_' + @SEGNAME + ' (
        ...
        -- เพิ่มคอลัมน์ Visit_group
        Visit_group VARCHAR(20)  -- เพิ่มคอลัมน์ Visit_group ที่ต้องการ
    );
    ';
...
-- ขั้นตอนที่ 2: แก้ไขเพิ่มคอลัมน์ที่  MKTCRM.SEG.CRM_SINGLE_VIEW_' + @SEGNAME + '
DECLARE @sql_add_column NVARCHAR(MAX);

SET @sql_add_column = N'
ALTER TABLE MKTCRM.SEG.CRM_SINGLE_VIEW_' + @SEGNAME + '
ADD 
...
EXEC sp_executesql @sql_add_column;


-- ขั้นตอนที่ 3: JOIN ข้อมูลจาก MKTCRM.SEG.CFM_HS
...
-- ขั้นตอนที่ 4: แก้ไขเพิ่มคอลัมน์ที่  MKTCRM.SEG.CRM_SINGLE_VIEW_' + @SEGNAME + ' เพิ่ม CFG
DECLARE @sql_add_column_2 NVARCHAR(MAX);

SET @sql_add_column_2 = N'
ALTER TABLE MKTCRM.SEG.CRM_SINGLE_VIEW_' + @SEGNAME + '
ADD 
    ...
    CFG_HOME_STORE_LOCATION NVARCHAR(100);   -- คอลัมน์ CFM_HOME_STORE_LOCATION
';

EXEC sp_executesql @sql_add_column_2;


-- ขั้นตอนที่ 5: update table
